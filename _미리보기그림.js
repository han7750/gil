...
// 가운데 큰 'road'
// 밑에 한 줄 글이 들어가므로(_미리보기그림.ps1) 조금 위로 올려 자리를 비워 둡니다
const 소문자 = 138;
const 낱말폭 = road폭(소문자);
const 결과 = road(Math.round((W - 낱말폭) / 2), 150, 소문자, 크림);

// 글자 밑에 도로 차선 다섯 칸 — 사이트 왼쪽의 줄무늬와 같은 표시입니다
const 칸 = 5, 칸폭 = 42, 사이칸 = 18;
const 시작 = Math.round((W - (칸 * 칸폭 + (칸 - 1) * 사이칸)) / 2);
for(let i = 0; i < 칸; i++) 둥근네모(시작 + i * (칸폭 + 사이칸), 결과.밑줄 + 52, 칸폭, 11, 6, 크림);
